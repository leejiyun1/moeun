...
class OrderCustomPackageItemSerializer(serializers.ModelSerializer):
    product = SimpleProductSerializer(read_only=True)
    subtotal = serializers.SerializerMethodField()

    class Meta:
        model = OrderCustomPackageItem
        fields = ["id", "product", "product_name", "price", "quantity", "sort_order", "subtotal"]
        read_only_fields = fields
...
class OrderCustomPackageSerializer(serializers.ModelSerializer):
    pickup_store = StoreSerializer(read_only=True)
    items = OrderCustomPackageItemSerializer(many=True, read_only=True)

    class Meta:
        model = OrderCustomPackage
        fields = [
            "id",
            "display_name",
            "package_policy_name",
