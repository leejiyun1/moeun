...
from apps.products.models import PackagePolicy, Product, ProductImage
    ...
    def test_drink_serialization_basic_fields(self):
        ...
        if not product:
            ...
            product = Product.objects.create(
                ...
                price=15000,
                description="테스트 상품",
                description_image_url="https://example.com/test-desc.jpg",
            )
...
class PackageSerializerTest(BaseSerializerTestCase):
    ...
    def test_package_serialization(self):
        ...
        self.assertIn("items", package_data)
...
class ProductCreationSerializerTest(BaseSerializerTestCase):
    ...
    def test_package_product_creation_allows_quantity_with_policy(self):
        """패키지 정책이 중복을 허용하면 quantity 기반 구성을 검증한다."""
        from apps.products.serializers.product.create import (
            PackageProductCreateSerializer,
        )
        from apps.products.tests.test_data import get_package_product_creation_data
        ...
        valid_data = get_package_product_creation_data([drinks[0].id, drinks[1].id, drinks[2].id])
        valid_data["package_info"] = {
            "name": "같은 술 3개 세트",
            "type": "CURATED",
            "policy_id": policy.id,
        ...
        self.assertEqual(serializer.validated_data["package_info"]["items"][0]["quantity"], 3)
