...
class AdminAPITest(BaseAPITestCase):
    ...
    def test_package_policy_create_with_allowed_products(self):
        ...
        self.client.force_authenticate(user=self.admin_user)
        url = reverse("products:v1:package-policies-manage-list")
        response = self.client.post(
            url,
            {
                ...
                "allowed_item_scope": PackagePolicy.AllowedItemScope.ALLOWED_SET,
                "discount_type": PackagePolicy.DiscountType.FIXED_AMOUNT,
                "discount_value": 3000,
                "allowed_product_ids": [str(product.id) for product in self.individual_products[:2]],
            },
            format="json",
        )

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        policy = PackagePolicy.objects.get(name="관리자 커스텀 3병 정책")
        self.assertEqual(policy.allowed_products.count(), 2)

    def test_package_policy_delete_deactivates_policy(self):
        """관리자 정책 삭제는 row 삭제가 아니라 비활성화로 처리된다."""
        self.client.force_authenticate(user=self.admin_user)
        policy = PackagePolicy.objects.create(name="비활성화 대상 정책")

        url = reverse("products:v1:package-policies-manage", kwargs={"pk": policy.pk})
        response = self.client.delete(url)

        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        policy.refresh_from_db()
        self.assertEqual(policy.status, PackagePolicy.Status.INACTIVE)

    def test_product_delete_deactivates_product(self):
        """관리자 상품 삭제는 row 삭제가 아니라 비활성화로 처리된다."""
        self.client.force_authenticate(user=self.admin_user)
        product = self.individual_products[0]
        initial_count = product.__class__.objects.count()

        url = reverse("products:v1:products-manage", kwargs={"pk": product.pk})
        response = self.client.delete(url)

        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(product.__class__.objects.count(), initial_count)
        product.refresh_from_db()
        self.assertEqual(product.status, product.Status.INACTIVE)

    def test_product_patch_updates_common_fields(self):
        """관리자 상품 수정은 command service 경로로 공통 판매 필드를 수정한다."""
        self.client.force_authenticate(user=self.admin_user)
        product = self.individual_products[0]
    ...
    def test_product_patch_rejects_invalid_discount(self):
        """관리자 상품 수정도 가격/할인 정합성을 검증한다."""
        self.client.force_authenticate(user=self.admin_user)
        product = self.individual_products[0]

        url = reverse("products:v1:products-manage", kwargs={"pk": product.pk})
        response = self.client.patch(url, {"original_price": 10000, "discount": 20000}, format="json")

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("discount", response.data)

    def test_product_patch_replaces_images(self):
        """관리자 상품 수정에서 이미지 목록을 교체할 수 있다."""
        self.client.force_authenticate(user=self.admin_user)
        product = self.individual_products[0]
    ...
    def test_individual_product_patch_updates_drink_info(self):
        """개별 상품 수정에서 술 상세 정보를 함께 수정할 수 있다."""
        self.client.force_authenticate(user=self.admin_user)
        product = self.individual_products[0]
        ...
        response = self.client.patch(
            url,
            {"drink_info": {"name": "관리자수정막걸리", "volume_ml": 500}},
            format="json",
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["drink"]["name"], "관리자수정막걸리")
        product.drink.refresh_from_db()
        self.assertEqual(product.drink.volume_ml, 500)

    def test_package_product_patch_updates_package_info(self):
        """패키지 상품 수정에서 패키지명과 구성 술을 함께 수정할 수 있다."""
        self.client.force_authenticate(user=self.admin_user)
        product = self.package_products[0]
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["package"]["name"], "관리자 수정 패키지")
        self.assertEqual(len(response.data["package"]["drinks"]), 3)
