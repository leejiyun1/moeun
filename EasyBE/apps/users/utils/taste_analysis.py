...
class TasteAnalysisService:
    ...
    def generate_analysis(profile: PreferTasteProfile) -> str:
        ...
        if learning_count == 0:
            analysis_parts.append("취향 테스트 결과를 기준으로 분석했어요.")

        elif learning_count < 3:
            analysis_parts.append("아직 후기 분석 데이터가 적어서 대략적인 취향만 파악할 수 있어요.")

        elif learning_count < 10:
            analysis_parts.append("최근 후기 분석 결과를 보니")

        else:
            analysis_parts.append(f"{learning_count}개의 후기 분석 결과를 반영해보면")
