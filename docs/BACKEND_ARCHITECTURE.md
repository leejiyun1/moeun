...
- 현재 코드는 픽업 필드를 기준으로 주문 전 정보를 관리하지만, 실제 운영 방식은 아직 확정되지 않았다.
- 커스텀 패키지 draft의 장바구니 UI와 주문 전 정보 입력 흐름은 아직 충분하지 않다.
- 운영 방식 확정 전까지 픽업 가능 날짜, 영업일, 마감 시간 같은 픽업 전용 정책을 더 확장하지 않는다.
- 운영 방식이 확정되면 `pickup_store`, `pickup_date`를 유지할지, 배송/픽업/문의형을 포괄하는 fulfillment 구조로 일반화할지 결정한다.
