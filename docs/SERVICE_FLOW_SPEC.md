...
- 장바구니 추가 시 수령 정보 없이도 임시 담기가 가능하다.
- checkout 전에는 주문 단위 수령 정보 누락이 차단된다.
...
## Flow 5. 사용자가 장바구니에서 주문 전 정보를 입력한다

### 목표

사용자가 담아둔 상품에 대해 주문 생성에 필요한 정보를 입력하고 주문 가능한 상태로 만든다.

현재 구현은 결제 전 장바구니 화면에서 주문 단위 수령 정보를 입력하는 기준이다. 주문 화면은 완성형 쇼핑몰 흐름으로 확장하되, 실제 판매 면허와 판매 주체가 확정되기 전까지 결제는 테스트 결제 모드로만 처리한다.
...
- 주문에는 `fulfillment_method`와 주문 단위 픽업 정보를 둔다.
- 일반 상품/패키지 항목별 픽업 정보는 기존 주문 snapshot 호환용으로만 유지하고, 신규 주문 생성 검증 기준은 `Order`의 수령 정보다.
...
- 장바구니 목록을 조회한다.
- 현재 구현 기준으로 픽업 매장 목록을 조회한다.
- 결제 전 선택한 주문 전체의 픽업 매장/날짜를 입력한다.
- 선택한 항목만 주문으로 넘긴다.
- `fulfillment_method=PICKUP`이면 주문 단위 픽업 정보가 없을 때 주문 버튼을 막는다.
...
- 백엔드에서 주문 단위 pickup 정보 누락을 프론트만 믿고 통과시키지 않는다.
...
- 매장 목록이 API에서 로드된다.
- 주문 단위 픽업 매장/날짜가 주문 생성 payload로 전달된다.
- `PICKUP` 주문에 픽업 정보가 없으면 주문이 막힌다.
- 선택한 항목만 주문으로 생성된다.
...
- 배송/문의형까지 확정되면 별도 fulfillment 모델 또는 배송지 snapshot 모델로 승격할지 결정한다.
...
- 사용자는 평점과 후기를 필수로 입력한다.
- 사용자는 이미지를 선택으로 첨부할 수 있다.
- 프론트 리뷰 작성 화면에서 맛별 점수, 입맛 적합도, 신뢰도, 태그 입력을 제거했다.
- 백엔드는 평점, 후기, 이미지만 리뷰 원본으로 저장한다.
- 리뷰 저장 시 취향 프로필을 자동 갱신하지 않는다.

남은 단계:

- 후기 저장 side effect를 `FeedbackCommandService`로 이동
- 후기 텍스트 분석 결과 저장 구조 설계
- 로컬 LLM 또는 규칙 기반 분석 인터페이스 설계
- 확실한 분석 결과만 입맛 프로필에 소폭 반영하는 학습 서비스 설계
...
사용자는 맛별 점수, 입맛 적합도, 신뢰도, 태그를 직접 입력하지 않는다.
...
- 상품 리뷰 수 갱신
- 후기 텍스트 분석 결과 저장
- 확실한 분석 결과만 학습 후보로 사용
- 평점과 후기 분석 방향이 신뢰 가능한 경우에만 취향 프로필 반영
...
- 사람이 직접 입력한 맛별 점수, 입맛 적합도, 신뢰도를 학습 근거로 삼지 않는다.

### 검증 기준

- 리뷰 저장과 분석은 분리된다.
- 리뷰 API 요청/응답에 맛별 점수, 입맛 적합도, 신뢰도, 태그가 없다.
- 평점과 후기 분석 신호가 일치할 때만 입맛 프로필에 반영된다.
- 어떤 리뷰 분석 결과가 어떤 입맛 축을 변경했는지 추적 가능하다.
...
- 태그 관리 화면에서 활성 태그와 비활성 태그를 분리해 표시
- 테스트 주문 목록 확인
- 시음 가능 여부 설정
- 폼 선택지를 상수로 관리
