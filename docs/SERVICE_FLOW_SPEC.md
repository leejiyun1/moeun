...
- 현재 구현 기준으로 각 장바구니 항목의 픽업 매장/날짜를 수정한다.
- 선택한 항목만 주문으로 넘긴다.
- 현재 구현 기준으로 선택한 일반 상품에 픽업 정보가 없으면 주문 버튼을 막는다.
...
- 픽업/배송/문의형 중 운영 주문 방식이 확정되기 전까지 주문 전 정보 UI를 더 키우지 않는다.
- 운영 주문 방식이 확정되면 `pickup_store`, `pickup_date`를 일반화할지 별도 fulfillment 모델로 승격할지 결정한다.
