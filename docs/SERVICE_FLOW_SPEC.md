...
- 프론트 리뷰 작성 화면에서 맛별 입력을 `입맛 적합도` 기준으로 전환했다.
- 사용자는 평점과 후기를 필수로 입력한다.
- 사용자는 6개 맛 축별로 내 입맛에 맞았는지 선택 입력할 수 있다.
- 백엔드는 입력된 맛 축 점수만 저장한다.
...
- 입맛 프로필 반영 로그 저장
- 후기 텍스트 분석 또는 로컬 LLM 연결은 보조 신호로 검토
...
6개 맛 축은 선택 입력이다. 사용자가 잘 모르겠는 항목은 건너뛴다.
...
- 리뷰 저장
- 상품 리뷰 수 갱신
- 입력된 입맛 적합도만 학습 후보로 사용
- 전체 평점과 입맛 적합도 방향이 신뢰 가능한 경우에만 취향 프로필 반영
...
- 입맛 적합도 점수를 술의 객관적 맛 강도처럼 취급하지 않는다.

### 검증 기준

- 리뷰 저장과 분석은 분리된다.
- 평점/후기/입맛 적합도 신호가 일치할 때만 입맛 프로필에 반영된다.
